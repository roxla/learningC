...
int open_img(char *fd_name, unsigned int *ebm_buf);
...
void showPic()
{
    ...
    const char arr[] = "1.ebm";
    length = sizeof(arr) / sizeof(arr[0]);
    // 申请堆内存
    unsigned int *ebm_buf = malloc(800 * 480 * 4);

    open_img(arr, ebm_buf);
...
int open_img(char *fd_name, unsigned int *ebm_buf)
{
    ...
    bzero(ebm_buf, 800 * 480 * 4); //清空堆空间

    read(img_fd, ebm_buf, 800 * 480 * 4);
    close(img_fd);
